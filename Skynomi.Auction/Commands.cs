...
namespace Skynomi.AuctionSystem
{
    public class Commands
    {
        ...
        public static void AuctionList(CommandArgs args)
        {
            ...
            if (listAuction.Count == 0 && !anyTarget)
            {
                args.Player.SendInfoMessage("No auctions found.");
                return;
            }
            else if (listAuction.Count == 0 && anyTarget)
            {
            ...
            else
            {
                ...
                foreach (var auction in listAuction)
                {
                    string playerName = auction["Username"];
        ...
        public static void AuctionAdd(CommandArgs args)
        {
            ...
            var data = AuctionSystem.Database.db.CustomVoid("SELECT ItemId, Amount FROM Auctions WHERE Username = @username AND ItemId = @itemid", new { username = args.Player.Name, itemid }, output: true);
            if (data.Count > 0)
            {
